...
def main():
    ...
    #Dyn = dynamical(atoms)
    ds = dipoleSpectrumMulti(atoms, Dyn, spectrum, y = 0.3)
    nds = (ds - np.min(ds)) / (np.max(ds) - np.min(ds))
    np.save("Danburite IR-PQEq", ds)
    ...
    #plt.show()
    plt.savefig("DanburiteIRM.png", dpi=500)
    return


def dipoleSpectrumMulti(atoms: MSONAtoms, Dyn: np.ndarray, spectrum: float, h: float = 1e-5, y: float = 1e-3) -> np.ndarray:
    c,d = vdos(Dyn)
    ...
    with Pool() as pool:
        dp = lambda v: dipolePartial(atoms, v, h)
        ddm = 1.602e-19 * np.array([ pool.map([ dipolePartial(atoms, v, h) for v in vibrations]) ])
