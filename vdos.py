...
def main():
    for file in os.listdir(f"{arr_root}"):
        #atoms = buildNumber(f"{cif_root}/{file}", 1000)
        Dyn = np.load(f"{arr_root}/{file}")
...
def vdosPlot(Dyn: np.ndarray, width: int = 50, title = ""):
    #Init
    c,d = vdos(Dyn)
    d = np.nan_to_num(d, 0)
    d = d[d != 0] 
...
def vdos(Dyn: np.ndarray, exclude: float = np.nan, save: str = False):
